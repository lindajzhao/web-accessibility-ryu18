...
var pluginName = "ik_sortable",
	modifier = navigator.platform.indexOf('Mac') > -1 ? 'Command' : 'Control', // modifier key depending on if windows or mac
	defaults = {
	...
	Plugin.prototype.init = function () {
		...
		$('<div/>') // aria-describedby attribute of the menu
		...
		plugin.items = $elem.children('li').each( function(i, el) {
			...
			$(el).attr({
				...
				'id': id + '_' + i,
				'role': 'listitem',
				'aria-label': $(el).text() + ' ' + (i + 1) + ' of ' + total + ' movable', // announce that lis are draggable
				'tabindex': i > 0 ? -1 : 0, // first li is focusable
			});
		})
	...
	Plugin.prototype.resetNumbering = function (plugin) {
		...
		plugin.items.each( function(i, el) {
			var $me = $(el);
			// Update labels to announce new position
			$me.attr({
